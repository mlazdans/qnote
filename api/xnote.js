		...
		return {
			xnote: {
				...
				async saveNote(root, fileName, note){
					...
					let d = xUtils.DateToXNoteDate(new Date(note.ts), prefs.dateformat);
				...
				async loadNote(root, fileName){
					...
					note.ts = xUtils.XNoteDateToDate(fileScriptableIO.read(32));
				...
				async getProfilePath() {
					return xUtils.getProfilePath().path;
				},
